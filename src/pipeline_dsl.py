...
class DomainPolicy:
    ...
    def is_repdigit(self, n: int) -> bool:
        """Check if n is a repdigit in self.base."""
        return BaseDigitOps(self.base).is_repdigit(n)

    @staticmethod
    def is_repdigit_base10(n: int) -> bool:
        """Legacy static method for base-10 repdigit check."""
        s = str(n)
        return len(set(s)) == 1
...
# =============================================================================
# BASE-AWARE DIGIT OPERATIONS (Layer B extension for multi-base support)
# =============================================================================

class BaseDigitOps:
    """Base-aware digit operations for arbitrary number bases.

    All operations work with digits in [0, base-1].
    Digit lists are MSB-first (most significant digit first).
    """
    ...
    def to_digits(self, n: int) -> List[int]:
        """Convert non-negative integer to digit list in self.base (MSB first)."""
        n = abs(n)
        if n == 0:
            return [0]
        digits = []
    ...
    def pad_to(self, digits: List[int], length: int) -> List[int]:
        """Zero-pad digit list to given length."""
        if len(digits) >= length:
            return digits
        return [0] * (length - len(digits)) + digits
    ...
    def reverse(self, n: int) -> int:
        return self.from_digits(self.to_digits(n)[::-1])
    ...
    def complement(self, n: int) -> int:
        """(base-1)-complement: replace each digit d with (base-1-d)."""
        digits = [(self.base - 1 - d) for d in self.to_digits(n)]
        return self.from_digits(digits)

    def kaprekar_step(self, n: int) -> int:
        """Kaprekar step: sort_desc - sort_asc, zero-padded to original digit count."""
        digits = self.to_digits(n)
        k = len(digits)
        desc = self.from_digits(sorted(digits, reverse=True))
        asc = self.from_digits(sorted(digits))
        diff = desc - asc
        # Zero-pad result back to k digits
        result_digits = self.pad_to(self.to_digits(diff) if diff > 0 else [0], k)
        return self.from_digits(result_digits)

    def truc_1089(self, n: int) -> int:
        """1089 trick: |n - rev(n)| + rev(|n - rev(n)|)."""
        if n <= 0:
            return 0
        rev = self.reverse(n)
        diff = abs(n - rev)
        if diff == 0:
            return 0
        return diff + self.reverse(diff)

    def add_reverse(self, n: int) -> int:
        return abs(n) + self.reverse(abs(n))

    def sub_reverse(self, n: int) -> int:
        return abs(abs(n) - self.reverse(abs(n)))

    def is_repdigit(self, n: int) -> bool:
        """Check if n is a repdigit in this base."""
        digits = self.to_digits(n)
        return len(set(digits)) == 1

    def swap_ends(self, n: int) -> int:
        digits = self.to_digits(n)
        if len(digits) <= 1:
            return n
    ...
    def rotate_left(self, n: int) -> int:
        ...
        if len(digits) <= 1:
            return n
        return self.from_digits(digits[1:] + digits[:1])
    ...
    def digit_pow(self, n: int, p: int) -> int:
        return sum(d ** p for d in self.to_digits(n))

    def digit_gcd(self, n: int) -> int:
        digits = [d for d in self.to_digits(n) if d != 0]
        if not digits:
            return 0
        result = digits[0]
        for d in digits[1:]:
    ...
    def digit_xor(self, n: int) -> int:
        result = 0
        for d in self.to_digits(n):
            result ^= d
        return result

    def narcissistic_step(self, n: int) -> int:
        digits = self.to_digits(n)
        k = len(digits)
        return sum(d ** k for d in digits)

    def digit_factorial_sum(self, n: int) -> int:
        return sum(math.factorial(d) for d in self.to_digits(n))
...
class OperationRegistry:
    ...
    def execute_with_base(self, name: str, n: int, base: int = 10) -> int:
        """Execute operation with explicit base. Falls back to base-10 impl if base==10."""
        if base == 10:
            return self.get_impl(name)(n)
        return self._execute_multibase(name, n, base)

    def _execute_multibase(self, name: str, n: int, base: int) -> int:
        """Execute operation using BaseDigitOps for non-base-10."""
        ops = BaseDigitOps(base)
        dispatch = {
            "reverse": ops.reverse,
            "sort_asc": ops.sort_asc,
            "sort_desc": ops.sort_desc,
            "rotate_left": ops.rotate_left,
            "rotate_right": ops.rotate_right,
            "swap_ends": ops.swap_ends,
            "digit_sum": ops.digit_sum,
            "digit_product": ops.digit_product,
            "digit_gcd": ops.digit_gcd,
            "digit_xor": ops.digit_xor,
            "digit_pow2": lambda n: ops.digit_pow(n, 2),
            "digit_pow3": lambda n: ops.digit_pow(n, 3),
            "digit_pow4": lambda n: ops.digit_pow(n, 4),
            "digit_pow5": lambda n: ops.digit_pow(n, 5),
            "digit_factorial_sum": ops.digit_factorial_sum,
            "narcissistic_step": ops.narcissistic_step,
            "complement_9": ops.complement,
            "kaprekar_step": ops.kaprekar_step,
            "truc_1089": ops.truc_1089,
            "add_reverse": ops.add_reverse,
            "sub_reverse": ops.sub_reverse,
            "collatz_step": OperationExecutor.collatz_step,  # base-independent
        }
        if name not in dispatch:
            raise KeyError(f"No multi-base implementation for: {name!r}")
    ...
    def execute_pipeline(self, pipeline: Pipeline, n: int, base: int = 10) -> int:
        for step in pipeline.steps:
            n = self.execute_with_base(step.op_name, n, base)
        return n
    ...
    def run_exhaustive(
        ...
        for n in domain.iterate():
            ...
            for t in range(1, max_iter + 1):
                val = reg.execute_pipeline(pipeline, val, base=domain.base)
                orbit.append(val)
                if val in seen:
