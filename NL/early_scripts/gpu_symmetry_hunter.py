...
GPU-versnelde symmetrie-ontdekking met CUDA kernels.
Benut RTX 4000 Ada voor massale parallelle evaluatie.
...
if HAS_CUDA:
    ...
    def kernel_1089_single(numbers, endpoints):
        """Single-shot 1089-truc: pas de truc EENMAAL toe."""
        idx = cuda.grid(1)
        if idx >= numbers.shape[0]:
            return
        n = numbers[idx]
...
class GPUSymmetryHunter:
    def __init__(self, config: GPUExperimentConfig = None):
        ...
        if not HAS_CUDA:
            raise RuntimeError("CUDA is vereist")
    ...
    def run_kaprekar_experiment(self, digit_range: Tuple[int, int] = (4, 4), count: int = None) -> Dict:
        ...
        print(f"\n{'='*60}")
        print(f"🔬 KAPREKAR EXPERIMENT: {digit_range[0]}-{digit_range[1]} cijfers")
        ...
        print(f"\n⏱️  GPU tijd: {elapsed*1000:.2f} ms")
        print(f"   Throughput: {count/elapsed/1e6:.2f} M getallen/sec")
        print(f"\n📊 Attractoren:")
    ...
    def run_1089_experiment(self, digit_range: Tuple[int, int] = (3, 3), constraint: str = "descending", count: int = None, single_shot: bool = True) -> Dict:
        ...
        print(f"\n{'='*60}")
        print(f"🔬 1089-TRUC EXPERIMENT: {digit_range[0]}-{digit_range[1]} cijfers")
        print(f"   Constraint: {constraint}")
        print(f"   Mode: {'single-shot' if single_shot else 'iteratief'}")
        print(f"{'='*60}")
    ...
    def run_happy_number_experiment(self, number_range: Tuple[int, int] = (1, 10000000), count: int = None) -> Dict:
        ...
        print(f"   Bereik: {number_range[0]:,} - {number_range[1]:,}")
        print(f"{'='*60}")
    ...
    def run_palindrome_experiment(self, digit_range: Tuple[int, int] = (2, 2), count: int = None, max_iter: int = 200) -> Dict:
        ...
        print(f"\n{'='*60}")
        print(f"🔬 PALINDROOM / LYCHREL EXPERIMENT")
        print(f"   Cijfers: {digit_range[0]}-{digit_range[1]}")
        print(f"{'='*60}")
    ...
    def discover_new_constants(self, digit_ranges: List[Tuple[int, int]] = None, operations: List[int] = None, samples_per_config: int = 100000) -> List[Dict]:
        ...
        print(f"\n{'='*70}")
        print(f"🔬 SYSTEMATISCHE CONSTANTEN ONTDEKKING")
        print(f"{'='*70}")
        ...
        for digit_range in digit_ranges:
            for op_code in operations:
                ...
                if endpoint_counts:
                    ...
                    if pct > 95:
                        print(f"🎯 CONSTANTE: {op_name} op {digit_range[0]}-cijferig → {top_val} ({pct:.1f}%)")
                        discoveries.append({
                            'operation': op_name,
                            'digit_range': digit_range,
        ...
        print(f"\n📊 Totaal ontdekt: {len(discoveries)} universele constanten")
        return discoveries
...
def main():
    ...
    # 1089 - met strikt aflopende cijfers (klassieke truc)
    print("\n" + "="*70)
    print("  1089-TRUC: STRIKT AFLOPENDE CIJFERS")
    print("="*70)
    for digits in [3, 4, 5, 6, 7, 8]:
        hunter.run_1089_experiment((digits, digits), constraint="descending", count=50_000, single_shot=True)
    
    # 1089 - met willekeurige getallen (werkt het ook?)
    print("\n" + "="*70)
    print("  1089-TRUC: WILLEKEURIGE GETALLEN")
    print("="*70)
    for digits in [3, 4, 5]:
