...
NIEUWE FEATURES in v6.0:
1. ML Pipeline Success Predictor - Voorspelt welke pipelines succesvol zullen zijn
2. Pipeline Feature Extraction - Rijke feature vectoren voor pipelines
3. Online Learning - Model verbetert tijdens de sessie
4. ML-based Quality Scoring - Geleerd model voor "interestingness"
...
Plus alle v5.0 features: Cyclus-detectie, Novelty bonus, Genetische mutatie, Attractor eigenschappen
"""
...
try:
    ...
    HAS_SKLEARN = False
    print(f"⚠️  scikit-learn niet beschikbaar ({e}) - ML features uitgeschakeld")
...
def get_pipeline_features(pipeline: List[int], result: Optional[Dict] = None) -> Dict[str, Any]:
    """Extraheert rijke features van een pipeline voor ML-modellen."""
    features = {
        'length': len(pipeline),
...
class MLPipelinePredictor:
    """Machine Learning model voor pipeline success prediction."""
    ...
    def _load_models(self):
        try:
            if (self.models_dir / "success_predictor.pkl").exists():
                ...
                print(f"📚 Success predictor model geladen", flush=True)
            ...
            print(f"⚠️ Kon modellen niet laden: {e}", flush=True)
    ...
    def _save_models(self):
        try:
            ...
            print(f"⚠️ Kon modellen niet opslaan: {e}", flush=True)
    ...
    def train_models(self):
        ...
        if n_samples < self.min_samples_for_training:
            return
        print(f"\n🧠 Training ML modellen met {n_samples} samples...", flush=True)
        try:
            ...
            print(f"   ⚠️ Training fout: {e}", flush=True)
...
class ExtendedResearchSessionV6:
    """ML-Enhanced Research Session met online learning."""
    ...
    def _load_previous_discoveries(self):
        ...
        conn.close()
        if self.op_alpha: print(f"📚 Geladen: {len(self.op_alpha)} operatie-statistieken", flush=True)
        if self.known_attractors: print(f"📚 Geladen: {len(self.known_attractors)} bekende attractoren", flush=True)
        if self.elite_pipelines: print(f"📚 Geladen: {len(self.elite_pipelines)} elite pipelines", flush=True)
        if self.ml_predictor and self.ml_predictor.training_data['pipeline_features']:
            print(f"📚 Geladen: {len(self.ml_predictor.training_data['pipeline_features'])} ML samples", flush=True)
...
| Metriek | Waarde |
|---------|--------|
| ML Model Versie | {self.ml_predictor.model_version if self.ml_predictor else 'N/A'} |
...
| ML-Gestuurde Pipelines | {self.ml_guided_pipelines} |
...
"""
        ...
        with open(self.config.report_path, 'w', encoding='utf-8') as f:
            f.write(report)
        print(f"\n📄 Rapport: {self.config.report_path}", flush=True)
    ...
    def run(self):
        ...
        print("█" * 70, flush=True)
        print(f"Session: {session_id} | Duur: {self.config.duration_minutes}m | ML: {self.config.ml_enabled and HAS_SKLEARN}", flush=True)
        print("█" * 70, flush=True)
        ...
        while time.time() < end_time:
            ...
            print(f"\n{'='*60}", flush=True)
            print(f"🔬 CYCLUS {self.cycle_count} | {elapsed:.1f}m/{self.config.duration_minutes}m{ml_info}", flush=True)
            print(f"   Ontdekkingen: {len(self.discoveries)} | Nieuw: {self.novel_discoveries} | ML-guided: {self.ml_guided_pipelines}", flush=True)
        ...
        print("\n" + "█" * 70, flush=True)
        print(f"  SESSIE COMPLEET: {len(self.discoveries)} ontdekkingen, {self.novel_discoveries} nieuw", flush=True)
        print(f"  ML-guided: {self.ml_guided_pipelines} pipelines, {self.ml_guided_successes} successen", flush=True)
        print("█" * 70, flush=True)
