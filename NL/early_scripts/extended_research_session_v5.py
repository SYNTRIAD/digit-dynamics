...
NIEUWE FEATURES in v5.0:
1. Cyclus-detectie - vindt cycli, niet alleen fixed points
2. Novelty bonus - beloont nieuwe, onbekende attractoren
3. Genetische mutatie - muteert succesvolle pipelines
4. Attractor eigenschappen - detecteert palindroom, priemgetal, perfecte macht, etc.

Uitgebreide onderzoekssessie die:
- Voortbouwt op eerdere ontdekkingen
- Creatief, iteratief en adaptief onderzoekt
- Alles vastlegt in database en rapport
- Automatisch dieper graaft bij interessante patronen
...
def is_palindrome(n: int) -> bool:
    """Check of n een palindroom is."""
    s = str(n)
    return s == s[::-1]

def is_perfect_power(n: int) -> Tuple[bool, Optional[Tuple[int, int]]]:
    """Check of n een perfecte macht is (a^b waar b >= 2)."""
    if n < 4:
        return False, None
...
def is_repdigit(n: int) -> bool:
    """Check of n een repdigit is (111, 222, 9999, etc.)."""
    s = str(n)
    return len(set(s)) == 1
...
def get_attractor_properties(n: int) -> Dict[str, any]:
    """Analyseer alle eigenschappen van een attractor."""
    props = {
...
@dataclass 
class Discovery:
    """Een ontdekking met uitgebreide eigenschappen."""
    pipeline: List[str]
    attractor: int
    convergence: float
    score: float
...
class ExtendedResearchSessionV5:
    """
    Uitgebreide onderzoekssessie v5.0 met:
    - Cyclus-detectie
    - Novelty search
    - Genetische mutatie
    - Attractor eigenschappen analyse
    """
    ...
    def __init__(self, config: SessionConfig = None):
        ...
        # NIEUW: Novelty archive - alle bekende attractoren
        self.known_attractors: Set[int] = set()
        self.novel_attractors: Set[int] = set()
        
        # NIEUW: Succesvolle pipelines voor mutatie
    ...
    def _init_database(self):
        """Initialize database met uitgebreide schema."""
    ...
    def _load_previous_discoveries(self):
        ...
        if self.known_attractors:
            print(f"📚 Geladen: {len(self.known_attractors)} bekende attractoren", flush=True)
        if self.elite_pipelines:
            print(f"📚 Geladen: {len(self.elite_pipelines)} elite pipelines voor mutatie", flush=True)
    
    def _save_discovery(self, discovery: Discovery, session_id: str):
        """Save discovery met uitgebreide eigenschappen."""
        conn = sqlite3.connect(self.config.db_path)
        c = conn.cursor()
        c.execute('''
    ...
    def _save_elite_pipeline(self, pipeline: List[str], score: float, attractor: int):
        """Save elite pipeline voor toekomstige mutatie."""
        conn = sqlite3.connect(self.config.db_path)
        c = conn.cursor()
        c.execute('''
    ...
    def mutate_pipeline(self, pipeline: List[int]) -> List[int]:
        """Muteer een pipeline op verschillende manieren."""
        ...
        if mutation_type == 'swap' and len(mutated) >= 2:
            # Verwissel twee operaties
            i, j = random.sample(range(len(mutated)), 2)
            mutated[i], mutated[j] = mutated[j], mutated[i]
        
        elif mutation_type == 'replace':
            # Vervang één operatie
    ...
    def crossover_pipelines(self, p1: List[int], p2: List[int]) -> List[int]:
        """Combineer twee pipelines (crossover)."""
    ...
    def generate_smart_pipeline(self, length: int = None) -> List[int]:
        """Genereer pipeline met geleerde kennis en genetische operaties."""
        if length is None:
        ...
        # NIEUW: Genetische mutatie van elite pipelines
        if strategy < 0.25 and self.elite_pipelines:
            # Muteer een elite pipeline
            ...
            # Crossover van twee elite pipelines
            ...
            # Thompson sampling met exploratie
    ...
    def detect_cycle_cpu(self, pipeline: List[int], start_val: int, max_iter: int = 100) -> Tuple[bool, int, List[int]]:
        """Detecteer cyclus in CPU voor verificatie."""
        seen = {}
        current = start_val
        sequence = [current]
        
        for step in range(max_iter):
            # Pas alle operaties toe
    ...
    def _apply_op_cpu(self, n: int, op_name: str) -> int:
        """Pas operatie toe op CPU (voor verificatie)."""
        if n <= 0:
            return 0
    ...
    def evaluate_pipeline(self, pipeline: List[int], start: int, end: int) -> Optional[Discovery]:
        """Evalueer pipeline op GPU met cyclus-detectie."""
        ...
        return Discovery(
            ...
            timestamp=time.time(),
            is_cycle=is_cycle,
            cycle_length=0,  # Zou CPU verificatie nodig hebben
    ...
    def _calculate_score(self, counter: Counter, total: int, pipeline: List[int],
        ...
        """Bereken score met alle nieuwe factoren."""
        if not counter:
            return 0.0
        ...
        # NIEUW: Attractor eigenschappen bonussen
    ...
    def explore_variations(self, base_discovery: Discovery) -> List[Discovery]:
        """Exploreer variaties van succesvolle pipeline met mutatie."""
        variations = []
            ...
            if result:
                ...
                if result.score > 45:
                    ...
                    if result.properties.get('is_palindrome'):
                        props.append("palindroom")
                    if result.properties.get('is_prime'):
                        props.append("priemgetal")
                    if result.properties.get('is_perfect_power'):
                        props.append(f"perfecte macht {result.properties.get('power_info')}")
                    if props:
                        print(f"      Eigenschappen: {', '.join(props)}", flush=True)
    ...
    def generate_report(self, session_id: str):
        """Genereer uitgebreid markdown rapport."""
...
**Nieuwe attractoren:** {self.novel_discoveries}  
**Cycli gevonden:** {self.cycles_found}
...
1. **Cyclus-detectie** - Vindt cycli, niet alleen fixed points
2. **Novelty search** - Beloont nieuwe, onbekende attractoren
3. **Genetische mutatie** - Muteert en combineert succesvolle pipelines
4. **Attractor eigenschappen** - Detecteert palindroom, priemgetal, perfecte macht
...
"""
        ...
        for i, d in enumerate(top, 1):
            ...
            if d.properties.get('is_palindrome'):
                props_list.append("palindroom")
            if d.properties.get('is_prime'):
                props_list.append("priemgetal")
            if d.properties.get('is_perfect_power'):
                props_list.append(f"perfecte macht")
...
## 🆕 Nieuwe Attractoren (Novelty Search)
...
*Gegenereerd door SYNTRIAD Extended Research Session v5.0*
"""
    ...
    def run(self):
        ...
        while time.time() < end_time:
            ...
            print(f"   Ontdekkingen: {len(self.discoveries)} | Nieuw: {self.novel_discoveries} | Cycli: {self.cycles_found}", flush=True)
        ...
        print(f"Nieuwe attractoren: {self.novel_discoveries}", flush=True)
        print(f"Cycli gevonden: {self.cycles_found}", flush=True)
