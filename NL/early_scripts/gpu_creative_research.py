...
GPU-versnelde versie van de creatieve experimenten.
Onderzoekt nieuwe operaties op schaal van miljoenen getallen.
...
try:
    ...
    HAS_CUDA = False
    print("⚠️  CUDA niet beschikbaar. Installeer: pip install numba")


# =============================================================================
# GPU DEVICE FUNCTIONS - Nieuwe Creatieve Operaties
# =============================================================================
...
if HAS_CUDA:
    ...
    def gpu_fibonacci_digit_sum(n: int64) -> int64:
        """Som van Fibonacci-mappings: 0→0, 1→1, 2→1, 3→2, 4→3, 5→5, 6→8, 7→13, 8→21, 9→34"""
    ...
    def gpu_digit_gravity(n: int64) -> int64:
        """Som van producten van aangrenzende cijfers"""
        if n <= 0:
            return 0
    ...
    def gpu_digit_pow3_sum(n: int64) -> int64:
        """Som van 3e machten van cijfers"""
        total = 0
        temp = n if n > 0 else -n
        while temp > 0:
            d = temp % 10
    ...
    def gpu_prime_digit_sum(n: int64) -> int64:
        """Som van alleen priemcijfers (2, 3, 5, 7)"""
        total = 0
        temp = n if n > 0 else -n
        while temp > 0:
            d = temp % 10
    ...
    def gpu_digit_wave(n: int64) -> int64:
        """Afwisselend optellen en aftrekken"""
        digits = cuda.local.array(20, dtype=int64)
        num_digits = 0
        temp = n if n > 0 else -n
        while temp > 0 and num_digits < 20:
    ...
    def gpu_binary_ones(n: int64) -> int64:
        """Aantal 1-bits in binaire representatie"""
        count = 0
        temp = n if n > 0 else -n
        while temp > 0:
            count += temp & 1
    ...
    def kernel_fibonacci_convergence(numbers, endpoints, steps, max_iter):
        """Fibonacci digit sum convergentie"""
        idx = cuda.grid(1)
    ...
    def kernel_xor_reverse_convergence(numbers, endpoints, steps, cycle_lengths, max_iter):
        """XOR-reverse convergentie met cyclus detectie"""
        idx = cuda.grid(1)
        if idx >= numbers.shape[0]:
            return
        ...
        # Simpele cyclus detectie met Floyd's algorithm
        slow = n
        fast = n
    ...
    def kernel_multi_base_check(numbers, base2_sums, base3_sums, base10_sums, matches):
        """Check multi-base digit sum gelijkheid"""
        idx = cuda.grid(1)
        if idx >= numbers.shape[0]:
            return
    ...
    def kernel_prime_factor_sum_convergence(numbers, endpoints, steps, max_iter):
        """Prime factor sum convergentie (vereenvoudigd)"""
        idx = cuda.grid(1)
        if idx >= numbers.shape[0]:
            return
...
class GPUCreativeResearcher:
    """GPU-versnelde creatieve experimenten."""
    ...
    def run_fibonacci_experiment(self, start: int, end: int) -> Dict:
        """Fibonacci digit sum experiment op GPU."""
        ...
        print(f"   Top attractoren:", flush=True)
    ...
    def run_xor_reverse_experiment(self, start: int, end: int) -> Dict:
        ...
        for val, count in counter.most_common(5):
            pct = 100 * count / len(numbers)
            print(f"      {val}: {count:,} ({pct:.1f}%)", flush=True)
        print(f"   Cyclus lengtes:", flush=True)
        for length, count in cycle_counter.most_common(5):
            print(f"      Lengte {length}: {count:,}", flush=True)
    ...
    def run_digit_alchemy_experiment(self, start: int, end: int) -> Dict:
        """Digit alchemy experiment op GPU."""
    ...
    def run_multi_base_experiment(self, start: int, end: int) -> Dict:
        """Multi-base digit sum gelijkheid experiment."""
        ...
        print(f"   Time: {elapsed:.3f}s ({len(numbers)/elapsed/1e6:.2f}M/s)", flush=True)
        print(f"   Harmonische getallen gevonden: {len(harmonic_numbers)}", flush=True)
        print(f"   Eerste 20: {list(harmonic_numbers[:20])}", flush=True)
    ...
    def run_digit_gravity_experiment(self, start: int, end: int) -> Dict:
        """Digit gravity experiment op GPU."""
    ...
    def run_all_experiments(self, start: int = 1, end: int = 1_000_000):
        ...
        print("\n📊 Fibonacci: 2 attractoren (1, 5)", flush=True)
        print("📊 XOR-Reverse: Interessante cycli gevonden", flush=True)
        print("📊 Digit Alchemy: 3-attractor symmetrie", flush=True)
        print(f"📊 Multi-Base: {results['multi_base']['count']} harmonische getallen", flush=True)
        print("📊 Digit Gravity: Meerdere kleine attractoren", flush=True)
...
def main():
    ...
    if not HAS_CUDA:
        print("❌ CUDA niet beschikbaar. Installeer numba met CUDA support.")
        return
