#!/usr/bin/env python3
"""
SYNTRIAD Quick Research - Fast iterative discovery with live output
"""
...
def evaluate_pipeline(pipeline, sample_size=500):
    """Fast pipeline evaluation with overflow protection."""
    endpoints = Counter()
    cycles = Counter()
...
def main():
    ...
    for iteration in range(5):
        ...
        log(f"\n  New discoveries: {iter_discoveries}")
        log(f"  Total: {len(discoveries)}")
    ...
    log("  RESULTS")
    log("█" * 60)
    log(f"Total discoveries: {len(discoveries)}")
