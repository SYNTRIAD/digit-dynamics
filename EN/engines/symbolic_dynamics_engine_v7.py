...
From semi-symbolic to true symbolic reasoning.

Three fundamental upgrades compared to v6.0:
  1. OPERATOR ALGEBRA — Formal operator properties, symbolic
     invariant prediction BEFORE sampling
  2. FIXED-POINT SOLVER — f(n) = n constraint-search + characterization
  3. META-THEOREM GENERATOR — Universal statements about operator classes
     with active falsification
  4. EMERGENT MECHANISM DISCOVERY — Co-occurrence clustering instead of templates
...
# =============================================================================
# LAYER 2: OPERATOR ALGEBRA
# =============================================================================

class AlgebraicProperty(Enum):
    """Formal algebraic properties of an operator."""
...
class OperatorProfile:
    """Complete formal profile of an operator."""
    name: str
    properties: Set[AlgebraicProperty] = field(default_factory=set)
    
    # Quantitative characteristics (exactly measured over exhaustive domain)
...
class OperatorAlgebra:
    """
    Formal algebra of digit operators.

    Computes operator properties EXACTLY over a large domain,
    and can then SYMBOLICALLY predict which pipelines will have
    which invariants.
    """
    ...
    def _compute_all_profiles(self):
        """Compute formal profiles for all operators."""

        # Use large domain for accuracy
    ...
    def _compute_composition_rules(self):
        """
        Compute which properties are preserved under composition.

        Key rule: if f preserves mod k AND g preserves mod k,
        then g∘f also preserves mod k.
        """
        
        # Properties that are preserved under composition
        COMPOSABLE = {
        ...
        for n1, p1 in self.profiles.items():
            for n2, p2 in self.profiles.items():
                ...
                # Monotone reducing also composes
                ...
                # Bounded output: if the last operator is bounded
    ...
    def predict_pipeline_invariants(self, pipeline: Tuple[str, ...]) -> Set[AlgebraicProperty]:
        """
        SYMBOLICALLY predict which invariants a pipeline has.

        Without sampling. Purely from operator algebra.
        """
        ...
        # Composition: accumulate properties
        # Start with properties of first operator
        ...
        for i in range(1, len(pipeline)):
            ...
            # Keep only properties that survive compositionally
            ...
            # Monotone: both must be monotone
    ...
    def predict_convergence(self, pipeline: Tuple[str, ...]) -> Dict:
        """
        Predict convergence behavior BEFORE sampling.

        Theoretical basis:
        - MONOTONE + BOUNDED → convergence guaranteed (well-ordering ℕ)
        - MOD_K preserving → attractor ≡ input (mod k)
        - ENTROPY_REDUCING + BOUNDED → convergence via information compression
        """
        ...
        # Theorem 1: Monotone + Bounded → convergence
        ...
        # Theorem 2: Bounded + Entropy-reducing → convergence
    ...
    def get_summary(self) -> str:
        """Generate summary of operator algebra."""
...
# =============================================================================
# LAYER 3: FIXED-POINT SOLVER
# =============================================================================

@dataclass
class FixedPointCharacterization:
    """Characterization of a fixed point."""
    value: int
    pipeline: Tuple[str, ...]
...
class FixedPointSolver:
    """
    Solve f(n) = n via constraint-search and algebraic analysis.

    Not brute-force over the entire domain, but targeted search.
    """
    ...
    def solve(self, pipeline: Tuple[str, ...],
        ...
        """
        Find fixed points f(n) = n.

        Strategy: use algebraic constraints to limit the search space.
        """
        
        fixed_points = []
        
        # Strategy 1: Direct search in bounded space
        # If pipeline is bounded, search only in bounded range
        search_max = domain[1]
        if predicted_properties and AlgebraicProperty.BOUNDED_OUTPUT in predicted_properties:
            # Output is bounded — search in smaller range
            search_max = min(10000, domain[1])
        
        # Strategy 2: Test candidates
        # Low range exhaustively
        ...
        # Strategy 3: Structured candidates (higher range)
        if search_max > 10000:
    ...
    def _characterize(self, n: int, pipeline: Tuple[str, ...]) -> FixedPointCharacterization:
        """Complete algebraic characterization of a fixed point."""
    ...
    def _generate_structured_candidates(self, domain: Tuple[int, int]) -> List[int]:
        """Generate structurally interesting candidates."""
        candidates = set()
...
# =============================================================================
# LAYER 3: META-THEOREM GENERATOR
# =============================================================================
...
class MetaTheorem:
    """
    A universal statement about a class of operator compositions.

    Not about a single pipeline, but about ALL pipelines with certain properties.
    """
    id: str
    statement: str
    formal: str
    
    # Which operator properties trigger this theorem
...
class MetaTheoremGenerator:
    """
    Generates universal theorems about operator classes.

    E.g.: "All pipelines with {MONOTONE, BOUNDED} converge in finite time."
    """
    ...
    def _generate_candidate_theorems(self):
        """Generate candidate theorems from operator algebra knowledge."""
    ...
    def test_all(self, n_test_pipelines: int = 50):
        """Test all candidate theorems with random pipelines."""
        ...
        for theorem in self.theorems:
            ...
            for _ in range(n_test_pipelines):
                # Generate pipeline that satisfies the antecedent
    ...
    def _generate_satisfying_pipeline(self, required: Set[AlgebraicProperty],
        ...
        """Generate pipeline that satisfies required properties."""

        # Find operators that contribute to required properties
        useful_ops = []
        ...
        # Generate pipeline of 2-3 operators
    ...
    def _test_consequent(self, pipeline: Tuple[str, ...], 
        ...
        """Test whether the consequent of a theorem holds for a pipeline."""
        
        numbers = random.sample(range(100, 99999), 2000)
        
        if theorem.consequent == "finite_convergence":
            # Test: does it converge?
            converged = 0
            for n in numbers[:500]:
                ...
                for step in range(200):
                    ...
                    if current == prev:
                        converged += 1
                        break
            return converged > 400  # >80% converges
        
        elif theorem.consequent == "mod9_attractor_constraint":
            # Test: attractor satisfies mod-9 constraint
            endpoints = Counter()
...
class EmergentMechanism:
    """A mechanism discovered via co-occurrence, not via template."""
    id: str
    name: str
...
class EmergentMechanismDiscovery:
    """
    Discover mechanisms via co-occurrence clustering.
    No templates — purely emergent.
    """
    ...
    def discover(self) -> List[EmergentMechanism]:
        """Discover emergent mechanisms via co-occurrence."""
        ...
        # Group by predicted invariant signature
        ...
        for signature, group in signature_groups.items():
            ...
            # Check if this group has consistently high dominance
            if avg_dom > 60:
        ...
        # Check for co-occurrence patterns not in signatures
        # Which observed properties frequently co-occur?
        obs_pairs = Counter()
        for obs in self.observations:
            for p1, p2 in itertools.combinations(sorted(obs["observed"]), 2):
                obs_pairs[(p1, p2)] += 1
        
        # Frequent pairs not already in a mechanism
...
class SymbolicDynamicsEngine:
    ...
    The leap from semi-symbolic to true symbolic reasoning.
    """
    ...
    def select_pipeline(self) -> Tuple[str, ...]:
        """Select pipeline with learned biases."""
    ...
    def explore_pipeline(self, pipeline: Tuple[str, ...]) -> Dict:
        """Complete 3-layer exploration."""
        
        # LAYER 2: Symbolic prediction BEFORE sampling
        prediction = self.algebra.predict_convergence(pipeline)
        predicted_props = prediction["predicted_properties"]
        
        # LAYER 1: Empirical verification
        ...
        # LAYER 3: Fixed-point analysis (if convergent)
        fixed_points = []
        ...
        # Register with emergent discovery
