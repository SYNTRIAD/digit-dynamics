...
A true autonomous researcher that:
1. Independently generates and tests new pipeline combinations
2. Analyzes basin-of-attraction structures
3. Identifies and classifies exceptions
4. Formulates hypotheses based on patterns
5. Detects algebraic reductions
6. Iteratively improves itself

This is no longer a brute-force explorer, but a discovery engine
that recognizes patterns and generates new hypotheses.
...
class DigitOp:
    """Base class for digit operations."""
...
@dataclass
class Hypothesis:
    """A mathematical hypothesis generated by the system."""
    id: str
    type: HypothesisType
    pipeline: Tuple[str, ...]
    claim: str
...
@dataclass
class BasinInfo:
    """Information about a basin of attraction."""
...
class AttractorGraph:
    """Complete attractor structure of a pipeline."""
    pipeline: Tuple[str, ...]
...
class BasinAnalyzer:
    """Analyzes basin-of-attraction structures."""
    ...
    def apply_pipeline(self, n: int, pipeline: Tuple[str, ...], max_iter: int = 500) -> Tuple[int, List[int]]:
        """Apply pipeline and return (endpoint, trajectory)."""
        trajectory = [n]
        seen = {n: 0}
        current = n
    ...
    def detect_cycle(self, n: int, pipeline: Tuple[str, ...], max_iter: int = 500) -> Tuple[Tuple[int, ...], int]:
        """Detect cycle and return (cycle_elements, steps_to_cycle)."""
        trajectory = [n]
        seen = {n: 0}
        current = n
    ...
    def analyze_basins(self, pipeline: Tuple[str, ...], 
        ...
        """Full basin-of-attraction analysis."""
    ...
    def _analyze_exceptions(self, exceptions: List[int], pipeline: Tuple[str, ...]) -> Dict:
        """Analyze why exceptions do not converge to the dominant attractor."""
        if not exceptions:
...
class AlgebraicDetector:
    """Detects algebraic patterns and relations."""
    ...
    def find_algebraic_relation(a: int, b: int) -> Optional[str]:
        """Find algebraic relation between two numbers."""
        relations = []
...
class HypothesisGenerator:
    ...
    def from_basin_analysis(self, graph: AttractorGraph) -> List[Hypothesis]:
        """Generate hypotheses from basin analysis."""
        hypotheses = []
    ...
    def from_algebraic_analysis(self, attractor: int, pipeline: Tuple[str, ...]) -> List[Hypothesis]:
        """Generate hypotheses from algebraic analysis."""
        hypotheses = []
...
class PipelineGenerator:
    """Generates new pipeline combinations intelligently."""
    ...
    def generate_structured(self, pattern: str = "reduce_transform") -> Tuple[str, ...]:
        """Generate pipeline with structure."""
    ...
    def generate_from_success(self) -> Optional[Tuple[str, ...]]:
        """Generate new pipeline based on successes."""
...
class AutonomousDiscoveryEngine:
    ...
    def test_hypothesis(self, h: Hypothesis, extended_domain: Tuple[int, int]) -> Hypothesis:
        """Test a hypothesis on an extended domain."""
        h.status = "testing"
    ...
    def analyze_exceptions_deeply(self, pipeline: Tuple[str, ...], 
        ...
        """In-depth analysis of exceptions."""
        print(f"\n{'='*70}")
