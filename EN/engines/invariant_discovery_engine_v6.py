...
Three layers:
  LAYER 1 - Empirical Dynamics (from v5.0)
  LAYER 2 - Structural Abstraction (NEW)
  LAYER 3 - Symbolic Reasoning (NEW)

This system:
  1. Detects algebraic invariants (not statistical, structural)
  2. Generates Conjectures as first-class objects
  3. Actively searches for counterexamples
  4. Minimizes complexity (MDL)
  5. Detects cross-domain isomorphisms
  6. Builds new conceptual categories
...
class InvariantType(Enum):
    ...
    CONTRACTIVE = "contractive"     # |f(n) - A| < |n - A| toward attractor A
    CONGRUENCE_CLASS = "congruence" # f preserves congruence class
    ENTROPY_REDUCING = "entropy"    # H(f(n)) < H(n)
    IDEMPOTENT = "idempotent"       # f(f(n)) == f(n)
    ABSORBING = "absorbing"         # once in A, stays in A


@dataclass
class Conjecture:
    """A conjecture as a first-class object."""
    id: str
...
class ConceptualCategory:
    """A new conceptual category discovered by the system."""
    id: str
    name: str                               # Generated name
    description: str                        # What it is
    defining_properties: List[str]          # Which invariants define it
    member_pipelines: List[Tuple[str, ...]] # Pipelines that belong to it
    member_attractors: List[int]            # Attractors in this category

    # Isomorphism
    isomorphic_to: List[str] = field(default_factory=list)

    # Quality
    cohesion: float = 0.0                   # How coherent the category is
    separation: float = 0.0                 # How distinctive
...
class InvariantMiner:
    """Detects algebraic invariants - structural, not statistical."""
    ...
    def mine_modular_invariants(self, pipeline: Tuple[str, ...],
        ...
        """Search for mod-k invariants: f(n) mod k == n mod k."""
        conjectures = []
    ...
    def mine_monotonicity(self, pipeline: Tuple[str, ...],
        ...
        """Detect monotonic reduction: f(n) < n."""
        conjectures = []
        numbers = random.sample(range(domain[0], domain[1] + 1),
                               min(sample_size, domain[1] - domain[0]))
    ...
    def mine_boundedness(self, pipeline: Tuple[str, ...],
        ...
        # Check if output is bounded regardless of input size
        upper_quartile = sorted(results)[int(0.99 * len(results))]
        input_range = domain[1] - domain[0]
        
        # If 99% of outputs are much smaller than input range
    ...
    def mine_contractivity(self, pipeline: Tuple[str, ...],
                            ...
                            domain: Tuple[int, int] = (100, 99999),
                            sample_size: int = 5000) -> List[Conjecture]:
        """Detect contractivity toward attractor."""
        numbers = random.sample(range(domain[0], domain[1] + 1),
                               min(sample_size, domain[1] - domain[0]))
        conjectures = []
    ...
    def mine_entropy_reduction(self, pipeline: Tuple[str, ...],
        ...
        """Detect systematic entropy reduction."""
        numbers = random.sample(range(domain[0], domain[1] + 1),
                               min(sample_size, domain[1] - domain[0]))
        conjectures = []
    ...
    def mine_all_invariants(self, pipeline: Tuple[str, ...],
        ...
        """Mine all types of invariants for a pipeline."""
        conjectures = []
...
class MechanismSynthesizer:
    """Generates mechanistic explanations from invariant combinations."""

    # Known mechanism templates
    MECHANISMS = {
    ...
    def synthesize(self, conjectures: List[Conjecture],
        ...
        """Synthesize mechanisms from conjecture combinations.
...
class CategoryBuilder:
    """Builds new conceptual categories from discoveries."""
    ...
    def register_pipeline(self, pipeline: Tuple[str, ...],
                           ...
                           attractor: Optional[int] = None,
                           dominance: float = 0.0):
        """Register a pipeline with its properties."""
    ...
    def discover_categories(self) -> List[ConceptualCategory]:
        """Discover categories by clustering pipeline profiles."""
        ...
        # Group pipelines based on shared mechanism categories
        ...
        # Group by invariant-type signature
        ...
        # Detect isomorphisms between categories
    ...
    def _detect_isomorphisms(self):
        """Detect structural isomorphisms between categories."""
...
class CounterexampleHunter:
    """Actively searches for counterexamples to conjectures."""
    ...
    def hunt(self, conjecture: Conjecture, 
        ...
        """Search for counterexamples with targeted strategies."""
    ...
    def _generate_candidates(self, strategy: str, 
        ...
        if strategy == "boundary":
            ...
            # Numbers with extreme digit variance
    ...
    def _is_counterexample(self, n: int, conjecture: Conjecture) -> bool:
        ...
        if conjecture.invariant_type == InvariantType.MODULAR:
            ...
            h_before = digit_entropy(n)
            h_after = digit_entropy(result)
            return h_after > h_before + 0.5  # Significant increase
...
class MDLScorer:
    """Minimal Description Length scoring for elegance."""
    ...
    def score(self, pipeline: Tuple[str, ...], dominance: float,
        ...
        Real discoveries = maximum structure, minimum complexity.
        """
        ...
        # Elegance bonus for short pipelines with high quality
        elegance_bonus = 0.0
...
class HomeostaticController:
    """Homeostatic meta-loop: the system regulates itself."""
    ...
    def select_pipeline(self, length: Optional[int] = None) -> Tuple[str, ...]:
        """Select pipeline based on learned biases."""
    ...
    def record_result(self, pipeline: Tuple[str, ...], score: float,
        ...
        # Track conjecture types that work
    ...
    def adapt(self):
        ...
        if avg_recent < avg_older * 0.8:
            # Performance declining -> explore more
...
class InvariantDiscoveryEngine:
    ...
    def explore_pipeline(self, pipeline: Tuple[str, ...]) -> Dict:
        """Full 3-layer exploration of a pipeline."""
